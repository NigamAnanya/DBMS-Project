 ...
 /*var tableToExcel = (function() {
    ...
    function ExportToExcel(type, fn, dl) {
        ...
        return dl ?
          ...
          XLSX.writeFile(wb, fn || ('Consolidated_Report_of_Cand_Ans.' + (type || 'xlsx')));
     }
</script>
<head>
		...
		<table id="consolidatedTable"
			...
			<tbody>
				<c:forEach items="${listOfUsers }" var="u">
					<tr>
						<th>${u.userid }</th>
						<c:forEach items="${listOfCandidateResponse }" var="cr">
							<c:if test="${u.userid == cr.userid }">
								<c:choose>
									<c:when
										test="${ cr.uans.toUpperCase() eq cr.cans.toUpperCase()}">
										<th>1</th>
									</c:when>
									<c:otherwise>
										<th>0</th>
